...
AddEventHandler('speakeasy:getCurrentBucket', function()
    local src = source
    local currentBucket = GetPlayerRoutingBucket(src)
    TriggerClientEvent('speakeasy:receiveBucket', src, currentBucket)
end)

-- Handle routing bucket restoration
RegisterServerEvent('speakeasy:restoreBucket')
AddEventHandler('speakeasy:restoreBucket', function(bucketId, spawns)
    local src = source
    SetPlayerRoutingBucket(src, bucketId or 0)

    -- If there are queued gather spawns, broadcast them to clients so props/guards are created in the main world.
    if spawns and type(spawns) == 'table' and #spawns > 0 then
        -- small delay to let client fully restore
        Citizen.SetTimeout(200, function()
            TriggerClientEvent('speakeasy:spawnGatherPickups', -1, spawns)
            print("Broadcasted " .. tostring(#spawns) .. " queued gather spawns from source " .. src)
        end)
    end
end)
...
Citizen.CreateThread(function()
    ExecuteSql("CREATE TABLE IF NOT EXISTS speakeasies (id INT AUTO_INCREMENT PRIMARY KEY, identifier VARCHAR(50), speakeasy_name VARCHAR(100) NOT NULL, owner_name VARCHAR(100) NOT NULL DEFAULT 'Unknown')", {})
...
AddEventHandler('speakeasy:assignInstance', function(speakeasyType)
    ...
    -- Save original bucket, then create a unique bucket ID per player-speakeasy combo
    ...
    -- Send teleport instruction to client, including original bucket so client can restore later
