...
local queuedGatherSpawns = nil

-- Receive bucket from server
RegisterNetEvent('speakeasy:receiveBucket')
AddEventHandler('speakeasy:receiveBucket', function(bucket)
                ...
                if data.interior then
                    ...
                    if exitDist < 2.5 then
                        ...
                        if PromptHasHoldModeCompleted(exitPrompt) then
                            ...
                            -- Send any queued gather spawns to the server so they are broadcast after the player is restored
                            TriggerServerEvent('speakeasy:restoreBucket', originalBucket or 0, queuedGatherSpawns)
                            queuedGatherSpawns = nil
                            Wait(500)
                            DoScreenFadeIn(500)
                            hasTeleported = false
                        end
...
AddEventHandler('speakeasy:teleportToInterior', function(data)
    -- Store original bucket if provided by server (safer than requesting from client-side)
    if data.originalBucket ~= nil then
...
AddEventHandler('speakeasy:triggerGather', function(speakeasyName)
    ...
    -- Instead of spawning supplies immediately (which would spawn them in the player's current routing bucket and
    -- cause them to disappear when the player changes instance), queue them to spawn after the player exits the
    -- speakeasy so they appear in the main world and are visible to other players.
    queuedGatherSpawns = {}
    for i, pickup in ipairs(speakeasy.supplyPickups) do
        table.insert(queuedGatherSpawns, {
            coords = pickup.coords,
            item = pickup.item,
            amount = pickup.amount,
            propHash = pickup.propHash
        })
    end
    TriggerEvent('vorp:TipRight', "Supplies queued and will spawn when you exit the speakeasy.", 5000)
    print("Queued supplies for " .. speakeasyName .. " to spawn on exit")
...
-- Spawn gather pickups in the main world (broadcast from server after player exits instance)
RegisterNetEvent('speakeasy:spawnGatherPickups')
AddEventHandler('speakeasy:spawnGatherPickups', function(spawns)
    if not spawns or type(spawns) ~= 'table' or #spawns == 0 then return end
    print("Received " .. tostring(#spawns) .. " gather spawns to create in world")

    -- create a local copy of pickups for this spawn event
    local spawned = {}
    ...
    -- spawn props, blips and guards
    for _, pickup in ipairs(spawned) do
        ...
        if not HasModelLoaded(pickup.propHash) then
            print("Failed to load model for spawn, using fallback")
        ...
        if DoesEntityExist(pickup.prop) then
            SetEntityAsMissionEntity(pickup.prop, true, true)
            Citizen.InvokeNative(0x58A850EAEE20FAA3, pickup.prop, true)
            Wait(500)
            FreezeEntityPosition(pickup.prop, true)
        else
            print("Failed to create world prop for gather at " .. tostring(pickup.coords))
        end
        ...
        if HasModelLoaded(guardModel) then
    ...
    TriggerEvent('vorp:TipRight', "Supplies spawned in the world! Check the blips to collect them.", 5000)

    -- Collection loop for these spawned pickups
    Citizen.CreateThread(function()
        local collectPrompt = PromptRegisterBegin()
        PromptSetControlAction(collectPrompt, 0xCEFD9220) -- E key
        PromptSetText(collectPrompt, CreateVarString(10, "LITERAL_STRING", "Collect Supply"))
        ...
        while active do
            ...
            local closestPickup = nil
            local closestDist = 1.5
            for _, pickup in pairs(spawned) do
                if pickup.prop and DoesEntityExist(pickup.prop) then
                    local dist = #(playerCoords - vector3(pickup.coords.x, pickup.coords.y, pickup.coords.z))
                    if dist < closestDist then
                        closestDist = dist
            ...
            if closestPickup then
                ...
                if dist < 1.5 then
                    ...
                    if PromptHasStandardModeCompleted(collectPrompt) then
                        TriggerServerEvent('speakeasy:collectSupply', closestPickup.item, closestPickup.amount)
                        if closestPickup.prop and DoesEntityExist(closestPickup.prop) then
                            DeleteObject(closestPickup.prop)
                            closestPickup.prop = nil
                        end
                        if closestPickup.blip then
